...
WORKERS = int(os.environ.get("ACT_WORKERS", "8"))
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as ex:
        for res in ex.map(conv_msgs, ids):
            ...
            if done % 500 == 0:
                ...
                print(f"  [msgs] {done}/{len(ids)}", file=sys.stderr)
    save(aggregate(events, users), len(ids), True)
    print(f"DONE: {len(ids)} convs, {len(events)} outbound rep messages", file=sys.stderr)
